...
- inline ghost-text completion for HTML, CSS, JavaScript, Anki special fields,
...
Inline completion appears after two characters by default. Press `Tab` to accept
the suggestion, `Alt+[`/`Alt+]` to cycle suggestions, or `Ctrl+Space` to request
one at any time. Settings are available from Anki's add-on configuration dialog.
